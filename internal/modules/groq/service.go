...
func GenerateTradeSignal(llm *openai.LLM, newsTitles []string, rates []models.CurrencyRate, binanceRate domain.BinanceRate) (models.TradeSignal, error) {
    ...
    for _, rr := range rates {
        if rr.Symbol == "USD" {
            usdPrice = rr.Price.StringFixed(2)
            break
        }
    }

    if usdPrice == "" {
        return models.TradeSignal{}, fmt.Errorf("official USD rate not found in database records")
    }
    ...
    systemPrompt := "Eres un algoritmo automatizado de análisis cuantitativo para fondos de cobertura. Tu única tarea es evaluar el INFORME FINANCIERO proporcionado por el usuario y generar una señal de trading en formato JSON estricto.\n\n" +
        "REGLAS MATEMÁTICAS Y DE MERCADO ABSOLUTAS:\n" +
        "1. Utiliza el valor exacto de 'bcv_usd_official' como la tasa oficial del dólar. Está estrictamente prohibido alucinar que falta este dato o inventar números externos.\n" +
        "2. Calcula la brecha real restando el valor numérico de 'bcv_usd_official' del precio 'buy' de Binance P2P.\n" +
        "3. REGLA DE CONTEXTO REAL: La tasa oficial del BCV es estrictamente referencial. Está prohibido asumir en el 'rationale' que es posible adquirir o comprar dólares libremente a esa tasa oficial del BCV. No califiques la brecha como una 'oportunidad de arbitraje directo para comprar al oficial', ya que la liquidez a ese precio es inexistente en el mercado real. Interpreta un spread amplio como presión cambiaria, escasez de divisas o riesgo macroeconómico elevado.\n" +
        "4. Redacta el 'rationale' en tercera persona neutral (ej: 'El sistema detecta...', 'Se recomienda...'). Debe ser un análisis técnico, frío y fluido de un solo párrafo sin ideas repetidas. Debe justificar la acción elegida basándose en el spread real y el sentimiento de riesgo de los titulares.\n" +
        "5. Coherencia de variables: Evalúa el spread y el contexto para determinar la acción correcta (\"BUY\", \"SELL\" o \"HOLD\") según tu análisis cuantitativo.\n\n" +
